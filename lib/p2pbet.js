...
function validateCreate({ typeId, side, stakeLamports, lockTs, nowMs, subject, subject2, minStake, maxStake, creator, duel }) {
  ...
  // ── DUEL — an open challenge: "my snake outlasts whoever takes this" ────────────────────────
  // Structurally the most rig-resistant shape the exchange can offer, which is why it gets its own
  // constraints rather than being a free-form outlast:
  //   * side A is ALWAYS the creator's own snake, side B is ALWAYS the acceptor's own snake, so
  //     each side is backing THEMSELVES to last longer.
  //   * Neither party can win by playing badly — throwing your own snake loses your own bet — so
  //     the self-harm rig that riggableSubjects exists to stop is impossible by construction.
  //   * Collusion is pointless: the two parties are direct opponents, and there is no third snake
  //     whose death either of them profits from.
  // subject2 is deliberately EMPTY at create: the opponent is whoever accepts, and it is set (and
  // verified to be the acceptor's own live snake) at accept time.
  if (duel) {
    if (typeId !== 'outlast') return 'a duel is an outlast wager';
    if (side !== 'A') return 'in a duel you always back your own snake';
    if (!creator) return 'missing creator';
    if (subject !== creator) return 'a duel must use your OWN snake';
    if (subject2) return 'a duel opponent is chosen when someone accepts';
