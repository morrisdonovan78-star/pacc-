...
function checkInvariant({ onChainBalance, wagerLiability, betLiability, accruedFee, payoutLamports = 0, txFee = 5000 }) {
  ...
  // The payout retires `payout` of a senior claim; the network fee is drawn from the fee (both cancel).
  const have = bal - payout - netFee;                                   // escrow after payout + tx fee
  const remainingLiability = Math.max(0, wL + bL + fee - payout - netFee);
  const ok = have >= remainingLiability;                                // ⇔ bal >= wL + bL + fee
  return {
    ok,
    have,
