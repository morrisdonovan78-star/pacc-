...
//   txFee           — network fee that also leaves escrow (default 5000, comes from the rent margin).
//
// After the payout: (onChainBalance - payout - txFee) >= (wagerLiability + betLiability + accruedFee - payout)
...
function checkInvariant({ onChainBalance, wagerLiability, betLiability, accruedFee, payoutLamports = 0, txFee = 5000 }) {
  ...
  // Players + bettors + the retained fee must ALL stay covered after this payout retires an equal
  // amount of a claim. Only the network fee needs headroom, supplied by the ever-present rent margin.
  const remainingLiability = Math.max(0, wL + bL + fee - payout);
  const have = bal - payout - netFee;         // what stays in escrow after the payout + network fee
  const ok = have >= remainingLiability;
  return {
    ok,
    have,
    need: remainingLiability,
    deficit: ok ? 0 : (remainingLiability - have),
  };
}
