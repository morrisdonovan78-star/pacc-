...
// Recruiter of the Week: single winner (top referrer) gets the flat prize.
function planRecruiterPayout(opts) {
  const { board = [], solPriceUsd = 0, escrowLamports = 0, floorLamports = 0, prizeUsd = 10 } = opts || {};
  const rows = board
    .map(r => ({ addr: String(r.addr || ''), name: r.name || '', recruits: Number(r.recruits) || 0 }))
    .filter(r => r.addr && r.recruits > 0)
    .sort((a, b) => b.recruits - a.recruits);

  if (rows.length === 0) return { winners: [], totalLamports: 0, bumped: false, ok: false, reason: 'no qualifying recruiters' };
  if (!(solPriceUsd > 0)) return { winners: [], totalLamports: 0, bumped: false, ok: false, reason: 'no SOL price' };

  const winners = [{ addr: rows[0].addr, name: rows[0].name, place: 1, usd: prizeUsd, lamports: usdToLamports(prizeUsd, solPriceUsd) }];
  return finalizePlan(winners, escrowLamports, floorLamports, false);
}
...
module.exports = { planBountyPayout, planRecruiterPayout, usdToLamports, LAMPORTS_PER_SOL };
