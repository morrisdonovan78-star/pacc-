 ...
 */
const kvSet      = (key, val, ttl)    => (ttl === undefined || ttl === null || ttl === '' || !Number.isFinite(Number(ttl)))
  ? _cmd(['SET', key, String(val)])
  : _cmd(['SET', key, String(val), 'EX', String(Math.max(1, Math.floor(Number(ttl))))]);
/*
 ...
 */
const kvSetNX    = (key, val, ttl)    => (ttl === undefined || ttl === null || ttl === '' || !Number.isFinite(Number(ttl)))
  ? _cmd(['SET', key, String(val), 'NX'])
  // A FLOAT ttl is rejected by Redis with the same "not an integer" error as `undefined` was, and on
  // this helper that costs money: the rejection returns null, null reads as "lock already held", and
  // the payout it guards is suppressed as a duplicate. Floored so it can never happen.
  : _cmd(['SET', key, String(val), 'EX', String(Math.max(1, Math.floor(Number(ttl)))), 'NX']);
